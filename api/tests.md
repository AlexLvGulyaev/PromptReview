...
### 14.1. Валидация на живом инстансе и A/B «новый путь vs старый путь»

**Дата:** 2026-08-30

| # | Тест | Результат |
|---|------|-----------|
| 1 | Прод: контейнер поднят, секция гайда прогнана запуском | `/health` (18001→8001) ok; API `backend: langchain_service, backend_available: true` — **PASS** |
| 2 | Прод E2E через живой API (S2S-ключ) | quality good 7.0, `token_usage {897/403/1300}`, 5112 мс; в логах контейнера `POST /process 200` — обработал сервис, не API — **PASS** |
| 3 | A/B «Ферма»: сервис vs in-process | poor 4.0 == poor 4.0 — вынос не изменил оценивание — **PASS** |
| 4 | A/B «Пифагор»: сервис vs in-process | good 7.0 vs fair 5.0 — различие в пределах вариативности LLM (temperature=0 не детерминирован), подтверждено и на повторных вызовах — **PASS** |
| 5 | Живое тестирование владельцем (Web UI/TG, прод на сервисе) | Пифагор 7.0 (1216 токенов), структурированный промпт 8.0 (2104 токена, многострочный ввод), короткий промпт «Плохо» 3.0 — поведение соответствует in-process — **PASS** |
| 6 | Дефект гайда, найденный запуском | host-порт 8001 занят другим проектом → `18001:8001` в compose, гайд обновлён — **PASS** |
