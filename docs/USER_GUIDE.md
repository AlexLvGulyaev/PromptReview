...
**Отличие:** Сервис **не выполняет** ваш промпт. Он анализирует его структуру, ясность, полноту и другие характеристики — как код-ревью для промптов.
...
**Время ответа:** 2–5 с на langflow, 7–13 с на langchain_service (замер 14.09.2026).
...
|-----------|------------|-----------|----------|
| **Web UI** | Ручная проверка через браузер | 2–5 с | Интуитивный |
| **Telegram Bot** | Быстрая проверка из мессенджера | 2–5 с | Мобильный |
| **REST API** | Интеграция в CI/CD, конвейеры | 2–5 с | Программный |
...
Через несколько секунд (замер: 2–5 с) вы получите структурированный ответ.
...
| `scores` | Оценки по 8 критериям (0–10) |
...
### Демо-режим (публичные демо-инстансы)

На публичной демо-версии Web UI в шапке виден **демо-бейдж**:

| Элемент | Описание |
|---------|----------|
| `Demo · осталось N/20 · M мин` | Остаток запросов сессии и время до истечения |
| Кнопка **⟲** | Начать новую демо-сессию |

Правила демо-режима:

- Демо-сессия ограничена по количеству запросов и по времени.
- Между запросами есть небольшой интервал — при слишком частой отправке появится сообщение с таймером.
- Когда квота исчерпана, ввод блокируется, и в шапке можно начать новую сессию кнопкой **⟲**.
- Если сессия истекла сама, обновите страницу или нажмите **⟲**.

Локальные развёртывания работают без этих ограничений (demo-режим выключен по умолчанию).
...
Сервис оценивает промпты по 8 критериям (0–10 баллов):
...
**Результат:** Вы получаете объективную оценку качества промпта за 2–5 с (замер).
...
A: 2–5 с на langflow, 7–13 с на langchain_service (замер 14.09.2026).
...
**Симптом:** Ответ приходит существенно дольше обычного (более 15 секунд).
