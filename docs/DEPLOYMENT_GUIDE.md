...
**Назначение:** Число повторных попыток LLM-вызова при временных ошибках (сеть, таймаут HTTP, 429, 5xx). 4xx-ошибки клиента не ретраятся. Работает через встроенный retry `ChatOpenAI` (OpenAI SDK).

**Бюджет времени:** общий `REQUEST_TIMEOUT_SECONDS` делится на все попытки (по умолчанию 30 сек / 3 = по 10 сек на попытку), поэтому суммарное время запроса не превышает лимит. Минимальный таймаут попытки — 5 сек.

**Обязательность:** ⚪ Нет

**По умолчанию:** `2` (0 — отключить retry)
...
- ✅ Учёт токенов LLM: поле `token_usage` в ответе `/review` + `tokens_*` в структурных логах + отображение во всех UI (Web UI и Telegram-бот)
- ✅ Исключение для S2S-клиентов при `DEMO_MODE=true`: запрос с `X-API-Key` = `API_KEY` (Telegram-бот, n8n) проходит без демо-токенов и демо-лимитов; бот передаёт ключ автоматически (см. секцию Telegram)
- ✅ Токен-строка в UI и исключение S2S не требуют новых переменных для существующих развёртываний (`RETRY_MAX_ATTEMPTS`, `API_KEY` — опциональные)
...
**2026-08-30 (v2.3):**
- ✅ Исправления по итогам Deployment Validation 2026-08-30 (валидация воспроизведения с нуля по этому документу в чистом окружении):
