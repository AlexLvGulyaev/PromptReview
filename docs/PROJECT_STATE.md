...
| Вопрос | Кто решает |
|--------|-----------|
| Закрывать ли инженерный долг пайплайна (токены, retry, метрики) перед витриной или зафиксировать как известные ограничения? | Владелец |
...
| Риск | Митигация |
|------|-----------|
| Отсутствует Deployment Validation — воспроизводимость развёртывания с нуля не подтверждена | Провести Validation по DEPLOYMENT_GUIDE.md в чистом окружении |
| В пайплайне нет учёта токенов и retry-логики; единственная защита — `REQUEST_TIMEOUT_SECONDS=30` | Инженерная задача (см. Next Steps P3) |
| Нет production-логов/метрик — поведение на реальном трафике не измерено | Фиксация как известного ограничения ввода в эксплуатацию |
